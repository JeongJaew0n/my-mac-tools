...
## 기각 — SwiftUI `MenuBarExtra`

처음엔 이걸로 만들었다. 아이콘 채움/비움으로 상태를 나타냈는데 **옆에 비교 대상이 없으면
채워졌는지 아닌지 구분이 안 된다**는 지적을 받고 초록 점으로 바꾸기로 했다.

그런데 `MenuBarExtra` 는 `NSStatusItem` 의 버튼을 내주지 않는다. 버튼에 접근하지 못하면
글리프를 template 으로 두면서 점만 색을 살리는 구성을 만들 수 없다. 그래서 `NSStatusItem`
직접 제어로 갈아탔다. 메뉴도 `NSMenu` 로 내려간다.
