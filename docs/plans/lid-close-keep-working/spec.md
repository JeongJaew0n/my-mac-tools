...
| Failure-mode | **이 프로세스가 켠 것**이고 암호 없이 끌 수 있을 때만 종료 시 자동 원복. 그 외에는 경고 다이얼로그. 종료 판단 직전에 커널 값을 다시 읽는다 | 자동 원복을 기각했던 이유가 "원복도 인증 창이 필요하다" 하나였고 sudoers 로 해소됐다. 다만 읽기는 주인을 가리지 않아도 **쓰기는 가려야 한다** — 다른 도구가 켜둔 것을 앱 종료가 말없이 꺼버리면 남의 상태를 망가뜨린다 |
