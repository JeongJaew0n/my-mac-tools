...
final class CaffeinateScanner: ObservableObject {
    ...
    private static func scan() -> [CaffeinateProcess] {
        ...
        for entry in all {
            namesByPID[entry.kp_proc.p_pid] = shortName(of: entry)
        }

        return all
            .filter { shortName(of: $0) == "caffeinate" }
            .map { entry -> CaffeinateProcess in
                let pid = entry.kp_proc.p_pid
                let ppid = entry.kp_eproc.e_ppid
                let arguments = argumentsOf(pid)
                let parentPath = executablePath(of: ppid)
                ...
                return CaffeinateProcess(
                    ...
                    parentName: friendlyName(of: ppid,
                                             path: parentPath,
                                             shortName: namesByPID[ppid] ?? ""),
                    parentPath: parentPath ?? "",
                    started: startDate(of: entry),
                    flags: parsed.flags,
    ...
    private static func shortName(of entry: kinfo_proc) -> String {
    ...
    private static func startDate(of entry: kinfo_proc) -> Date {
    ...
    private static func friendlyName(of pid: pid_t, path: String?, shortName: String) -> String {
    ...
    /// 실행 파일의 전체 경로. 다른 사용자 소유면 nil.
    ///
    /// 짧은 이름만으로는 주인을 알 수 없다. Claude Code 가 띄운 `caffeinate` 의 부모는
    /// 짧은 이름이 `2.1.276` 이라 아무 정보가 없지만, 경로는
    /// `/Users/…/share/claude/versions/2.1.276` 이라 한눈에 읽힌다.
